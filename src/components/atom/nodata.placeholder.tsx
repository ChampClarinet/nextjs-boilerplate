...
/** Props for the NodataPlaceholder component. */
export interface NodataPlaceholderProps {
  text?: string;
  color?: string;
}
/** Renders a no-data illustration with optional label and color. */
const NodataPlaceholder: FC<NodataPlaceholderProps> = ({ text = "ไม่มีรายการ", color }) => {
  return (
    <div className="flex min-h-[min(100%,400px)] w-full flex-col items-center justify-center gap-4 text-8xl select-none">
      <Nodata fill={color} />
      <h2 className="mx-auto text-xl" style={{ color }}>
        {text}
      </h2>
    </div>
  );
...
/** Props for the Nodata icon component. */
interface NodataProps {
  fill?: string;
}
/** Renders the no-data SVG icon. */
const Nodata: FC<NodataProps> = ({ fill = "currentColor" }) => {
  return (
    <svg
      className="svg-icon-nodata"
      style={{
        width: "1em",
        height: "1em",
